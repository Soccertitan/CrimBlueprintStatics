...
TArray<float> UCrimMathStatics::ExponentialDistributionRandomSampleSize(float Mean, int32 SampleSize,
	bool bUseLowerBounds, float LowerBounds, bool bUseUpperBounds, float UpperBounds)
{
...
void UCrimMathStatics::ClampPointWithinCone(const FVector& ConeOrigin, const FVector& ConeDirection,
	float ConeHalfAngleDegrees, FVector& TargetPoint, bool& bTargetPointAdjusted)
{
...
bool UCrimMathStatics::IsInCone(const FVector& ConeOrigin, const FVector& ConeDirection, float ConeHalfAngle,
	const FVector& Point)
{
...
float UCrimMathStatics::DistanceBetweenActors2D(AActor* Actor1, AActor* Actor2)
{
	if (Actor1 && Actor2)
	{
...
bool UCrimMathStatics::IsPointInsidePolygon(const FVector2D& TestPoint, const TArray<FVector2D>& PolygonPoints)
{
...
TArray<FVector> UCrimMathStatics::GetEquidistantTargetLocationsInCone(const FVector& InMiddleTargetLocation,
	const FVector& InCenterLocation, float InDistBetweenTargets, int32 InNumTargets)
{
...
float UCrimMathStatics::ExponentialDistributionRandom(float Mean, bool bUseLowerBounds, float LowerBounds,
	bool bUseUpperBounds, float UpperBounds)
{
	constexpr float RegularizationFactor = 1.f;
...
float UCrimMathStatics::RandomNegateFloat(float InFloat)
{
	return FMath::FRandRange(0.f, 1.f) * -1.f;
}

bool UCrimMathStatics::LineIntersect(const FVector2D& L1Start, const FVector2D& L1End, const FVector2D& L2Start,
	const FVector2D& L2End, FVector2D& Intersect, float Tolerance)
{
...
float UCrimMathStatics::AngleBetweenVectors(const FVector& VectorA, const FVector& VectorB)
{
